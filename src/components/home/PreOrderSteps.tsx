...
export function PreOrderSteps() {
  return (
    <section aria-labelledby="preorder-heading" className="bg-brand-dark py-16">
          ...
          {STEPS.map(({ step, title, desc }, idx) => (
            <div key={step} className="relative flex flex-col gap-4">
              ...
              {idx < STEPS.length - 1 && (
                <div
                  aria-hidden="true"
                  className="absolute left-[calc(50%+2rem)] top-6 hidden h-px w-[calc(100%+1.5rem)] bg-brand-accent/30 lg:block"
                />
              )}

              {/* Step number circle */}
              <div className="relative flex h-12 w-12 items-center justify-center rounded-full border-2 border-brand-accent bg-brand-dark">
                <span className="font-mono text-sm font-bold text-brand-accent">{step}</span>
              </div>
