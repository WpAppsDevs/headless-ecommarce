...
export function WholesaleBanner() {
  return (
    <section aria-labelledby="wholesale-heading" className="relative overflow-hidden bg-[#5C2A3C] py-16">
      {/* Decorative pattern */}
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 opacity-5">
        <div className="absolute -right-32 -top-32 h-96 w-96 rounded-full bg-[#C4867A]" />
        <div className="absolute -bottom-32 -left-32 h-96 w-96 rounded-full bg-[#C4867A]" />
      </div>
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center gap-10 text-center lg:flex-row lg:text-left">
          ...
          <div className="flex-1">
            <span className="inline-block rounded-full border border-[#C4867A]/40 bg-[#C4867A]/10 px-4 py-1 text-xs font-semibold uppercase tracking-widest text-[#C4867A]">
              Wholesale & Catalog
            </span>
            <h2 id="wholesale-heading" className="mt-4 font-serif text-3xl font-bold text-white sm:text-4xl">
              Grow Your Business with<br />
              <span className="text-[#C4867A]">Our Catalog Collections</span>
            </h2>
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-3 lg:justify-start">
              <Link
                href="/products"
                className="inline-flex items-center gap-2 rounded-full bg-[#C4867A] px-7 py-3.5 text-sm font-semibold text-[#7C3D52] transition hover:bg-[#A8685E]"
              >
                View Catalog
              </Link>
              <a
          ...
          <div className="grid w-full grid-cols-2 gap-4 sm:max-w-xs lg:max-w-none lg:grid-cols-4 lg:w-auto">
            {STATS.map(({ value, label }) => (
              <div key={label} className="flex flex-col items-center rounded-2xl bg-white/5 px-5 py-5 ring-1 ring-white/10">
                <span className="font-serif text-2xl font-bold text-[#C4867A]">{value}</span>
