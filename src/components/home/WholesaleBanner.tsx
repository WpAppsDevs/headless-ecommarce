...
export function WholesaleBanner() {
  return (
    <section aria-labelledby="wholesale-heading" className="relative overflow-hidden bg-brand-dark py-16">
      {/* Decorative pattern */}
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 opacity-5">
        <div className="absolute -right-32 -top-32 h-96 w-96 rounded-full bg-brand-accent" />
        <div className="absolute -bottom-32 -left-32 h-96 w-96 rounded-full bg-brand-accent" />
      </div>
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center gap-10 text-center lg:flex-row lg:text-left">
          ...
          <div className="flex-1">
            <span className="inline-block rounded-full border border-brand-accent/40 bg-brand-accent/10 px-4 py-1 text-xs font-semibold uppercase tracking-widest text-brand-accent">
              Wholesale & Catalog
            </span>
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-3 lg:justify-start">
              <Link
                href="/products"
                className="inline-flex items-center gap-2 rounded-full bg-brand-accent px-7 py-3.5 text-sm font-semibold text-brand-dark transition hover:bg-brand-accent-hover"
              >
                View Catalog
              </Link>
              <a
