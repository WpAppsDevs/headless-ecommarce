...
// ---------------------------------------------------------------------------
// Wishlist product card (trimmed version — just image + name + remove + cart)
// ---------------------------------------------------------------------------
...
function GuestWishlistView({ savedCount }: { savedCount: number }) {
  return (
    <div className="flex flex-col items-center justify-center gap-6 py-24 text-center">
      ...
      <div className="space-y-2">
        <h2 className="text-xl font-semibold text-zinc-900">Sign in to view your wishlist</h2>
        {savedCount > 0 ? (
          <p className="text-sm text-zinc-500">
            You have {savedCount} item{savedCount !== 1 ? 's' : ''} saved.
            <br />
...
// ---------------------------------------------------------------------------
// Empty state (authenticated, empty wishlist)
// ---------------------------------------------------------------------------
...
export default function WishlistPage() {
  ...
  const { hydrated, isAuthenticated, logout } = useAuthStore();
  const { remove, count: storeCount } = useWishlistStore();
    ...
    async function loadWishlist() {
      try {
        ...
        if (
          ...
          (e.code === 'session_expired' || e.code === 'invalid_token' || e.code === 'no_token')
        ) {
          await logout();
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        {/* Guest view */}
        {ready && !error && !isAuthenticated && (
          <GuestWishlistView savedCount={guestIds.length} />
        )}

        {/* Authenticated — empty */}
        {ready && !error && isAuthenticated && products.length === 0 && (
          <EmptyWishlist />
        )}
